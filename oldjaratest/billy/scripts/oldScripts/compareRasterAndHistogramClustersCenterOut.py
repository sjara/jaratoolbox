...
ephysRoot='/home/billywalker/data/ephys/test014/'
ephysSessionArray = ['2015-01-09_16-41-49']#['2014-12-19_14-36-45','2014-12-22_11-42-00','2014-12-23_15-50-14','2014-12-24_17-11-53','2014-12-26_18-07-12']#'2014-12-20_22-37-24','2014-12-26_18-07-12'

#subject = 'test014'
experimenter = 'santiago'
paradigm = '2afc'
sessionstrArray = ['20150109a']#['20141219a','20141222a','20141223a','20141224a','20141226a']#'20141220a','20141226a'
...
for cellID in range(0,numOfCells):
	...
	if (behavSession != oneCell.behavSession):
		...
		incorrectLeftward = incorrect*leftward
                ...
                #This gives an array with the times in seconds from the start of the behavior paradigm of when the sound was presented for each trial
                soundStartTimes = bdata['timeTarget']
		...
		##################################################################################################################################################################################
                ...
                ###############################################################################################################################################################################
                centerOutTimes = bdata['timeCenterOut']
                ######################################################################################################################################################################################
	...
	eventOnsetTimes = eventTimes[eventOnset == 1] #This gives only the times of the onset of the channel you want
        ...
        correctedCenterOutTimes = np.empty(len(centerOutTimes))
        # -- Find the timing difference between the behavior times and the ephys times and add the behavior times --
        for indTime,soundTime in enumerate(soundStartTimes):
            correctedCenterOutTimes[indTime] = centerOutTimes[indTime] + (eventOnsetTimes[indTime] - soundTime)
        

        centerOutTimesTrials1 = correctedCenterOutTimes[trialsToUse1==1]
        centerOutTimesTrials2 = correctedCenterOutTimes[trialsToUse2==1]

	eventOnsetTimesTrials1 = centerOutTimesTrials1
	eventOnsetTimesTrials2 = centerOutTimesTrials2 
	...
	title(ephysSession+' TT{0}'.format(oneCell.tetrode)+' cluster{0}'.format(oneCell.cluster))
	xlabel(timeLabeling)
	...
	filename = 'psy_%s_%s_%s_%s_%s.%s'%(subject,behavSession,tetrodeClusterName,nametrialsToUse,alignmentName,figformat)
	fulloutputDir = outputDir+subject+'/'+'cluster_centerOut'+'/'
