...
minFileName = 'quality_repsonsive_cluster' #name of the file to put the copied files in
################################################################################################
################################################################################################
...
copyBlockToDir = '/home/billywalker/Pictures/raster_block/'+subject+'/'+minFileName+'/'
...
for cellID in range(0,numOfCells):
    ...
    if ((abs(float(maxZDict[behavSession][midFreq][clusterNumber])) >= minZVal) & (ISIDict[ephysSession][tetrode-1][cluster-1] <= maxISIviolation)):
        for freq in minTrialDict[behavSession]:
            os.system('cp /home/billywalker/Pictures/raster_hist/%s/%s/rast_%s_%s_%s_T%sc%s.png /home/billywalker/Pictures/raster_hist/%s/%s/' % (subject,freq,subject,behavSession,freq,str(tetrode),str(cluster),subject,minFileName))
            os.system('cp /home/billywalker/Pictures/raster_block/%s/block_%s_%s_%s_T%sc%s.png /home/billywalker/Pictures/raster_block/%s/%s/' % (subject,subject,behavSession,freq,str(tetrode),str(cluster),subject,minFileName))
